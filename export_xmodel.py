...
def save(self, context, filepath, **kwargs):
    # Set active object if none selected or not a mesh
    if not context.object or context.object.type != 'MESH':
        ...
        if mesh_objects:
            ...
            # If no meshes but armatures exist and use_armature is True, allow export of armature only
            if kwargs.get('use_armature', True):
                armatures = [obj for obj in context.view_layer.objects if obj.type == 'ARMATURE']
                if armatures:
                    context.view_layer.objects.active = armatures[0]
                    # Proceed with armature-only export
                else:
                    return "No mesh or armature objects to export"
            else:
                return "No mesh objects to export"

    last_mode = context.object.mode

    use_armature_pose = False  # Disabled param for now
    ...
    try:
        _, _, _, filepath, _ = shared.process_filepath(filepath, target_format_key='target_format', default_format='XMODEL_BIN', **kwargs)
    ...
    armature, objects = shared.gather_exportable_objects(self, context, kwargs.get('use_selection', False), kwargs.get('use_armature', True))
    print(f"[DEBUG] Exportable objects: {[obj.name for obj in objects]}")

    # If no meshes but armature exists (armature-only export)
    if not objects and armature is not None:
        print("[DEBUG] Exporting armature only, no meshes found.")
        objects = []
    ...
    if kwargs.get('apply_unit_scale', True):
        global_scale = shared.apply_cm_to_inch_scale(global_scale)
    ...
    if objects:
        for obj in objects:
            ...
            try:
                ...
                shared.add_warning(f"RUNTIME ERROR getting object '{obj.name}'s mesh: {e}")
                continue
            ...
            print(f"[DEBUG] Added mesh '{mesh.name}' to export list")

    # Handle armature bones always (even if no meshes)
    if kwargs.get('use_armature', True) and armature is not None:
        ...
        for idx, bone in enumerate(armature.data.bones):
            parent_idx = bone_table.index(bone.parent.name) if bone.parent and bone.parent.name in bone_table else -1
            model_bone = XModel.Bone(bone.name, parent_idx)
            mark_cosmetic(model_bone, bone.name)

            if idx == 0:
                model_bone.matrix = [(1, 0, 0), (0, 1, 0), (0, 0, 1)]
                model_bone.offset = (0, 0, 0)
            else:
                mtx = (armature_matrix @ bone.matrix_local).to_3x3().transposed()
                model_bone.matrix = [tuple(mtx[i]) for i in range(3)]
                model_bone.offset = tuple((armature_matrix @ bone.head_local) * global_scale)

            model.bones.append(model_bone)
    else:
        dummy_name = "tag_origin"
        dummy_bone = XModel.Bone(dummy_name, -1)
        dummy_bone.offset = (0, 0, 0)
        dummy_bone.matrix = [(1, 0, 0), (0, 1, 0), (0, 0, 1)]
        model.bones.append(dummy_bone)
        bone_table = [dummy_name]

    weight_threshold = 0.0 if not kwargs.get('use_weight_min', False) else kwargs.get('use_weight_min_threshold', 0.010097)

    # Convert meshes to xmodel format if any
    for mesh in meshes:
        print(f"[DEBUG] Converting mesh '{mesh.mesh.name}' to XModel format")
        mesh.add_weights(bone_table, weight_threshold)
        model.meshes.append(mesh.to_xmodel_mesh(
    ...
    print(f"[DEBUG] Writing output to {filepath}")
